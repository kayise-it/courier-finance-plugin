...
if (isset($_GET['selected_ids']) || isset($_POST['bulk_ids'])) {
    ...
    if (!empty($selected_ids_array)) {
        ...
        if (!empty($waybills)) {
            ...
            // Calculate totals BEFORE starting output buffer (sum of waybill invoice amounts + SADC charges)
            $subtotal = 0.0;
            foreach ($waybills as $waybill) {
                $subtotal += floatval($waybill->product_invoice_amount ?? 0);
            }
            
            // Add SADC charges to subtotal if applicable
            if (isset($total_sadc) && $total_sadc > 0) {
                $subtotal += floatval($total_sadc);
            }

            // Calculate VAT on the combined subtotal
...
<table width="100%" cellpadding="3" cellspacing="0" style="max-width:700px; margin:0 auto; font-family:Arial,sans-serif;font-size:12px;color:#333;">
  <tr>
    <td colspan="2">
      ...
      <table width="100%" cellpadding="0" cellspacing="0" style="margin-bottom:5px; border-collapse:collapse;">
        <tr>
        ...
        <tr class="scolor" style="color:#fff;font-size:11px;">
          <th align="left" class="thr" style="padding:8px 6px;">Description</th>
          <th class="thr" align="left" style="padding:8px 6px;">Waybill #</th>
          <th class="thr" align="center" style="width: 100px; padding:8px 6px;">Dimensions (cm)</th>
          <th class="thr" align="center" style="width: 80px; padding:8px 6px;">Mass (kg)</th>
          <th class="thr" align="center" style="width: 80px; padding:8px 6px;">Volume (m³)</th>
          <th class="thr" align="right" style="width: 120px; padding:8px 6px;">Amount (R)</th>
          <th class="thr" align="right" style="width: 120px; padding:8px 6px;">Subtotal (R)</th>
        </tr>
        ...
        <?php if ($can_see_prices && !empty($total_sadc) && !empty($sadc_count)): ?>
          <tr style="font-size:13px; background:#f9fafb; border-bottom:1px solid #e0e7ef;">
            <td class="cellStyle fstCol">
            ...
            <td class="cellStyle aligncenter" colspan="3">
              <?= intval($sadc_count) ?>
            </td>
            <td class="cellStyle" style="text-align:right;">
              <?= number_format($sadc_unit_price, 2) ?>
            </td>
            <td class="cellStyle" style="text-align:right; font-weight: 700;">
              <?= number_format($total_sadc, 2) ?>
            </td>
          </tr>
        <?php endif; ?>
        ...
        <tr class="rowTotal pcolor">
          ...
          <td class="cellStyle" colspan="5" style="text-align:right; font-size:18px; font-weight:700;"></td>
          <td class="cellStyle" style="text-align:right; font-size:18px; font-weight:700;">
            <?php 
        ...
        <tr class="rowTotal pcolor">
          <td class="cellStyle" style="font-size:18px; font-weight:700;">TOTAL</td>
          <td class="cellStyle" colspan="5" style="text-align:right; font-size:18px; font-weight:700;"></td>
          <td class="cellStyle" style="text-align:right; font-size:18px; font-weight:700;">N/A</td>
        </tr>
        <?php endif; ?>
      </table>
