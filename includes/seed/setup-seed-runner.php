...
if (!function_exists('kit_prepare_seed_sql_for_execution')) {
    ...
    function kit_prepare_seed_sql_for_execution(string $sql_content, wpdb $wpdb): string
    {
        ...
        $created_by_user_id = 1593;
        if ($wpdb->prefix === 'wp_') {
            $thando_user = function_exists('get_user_by') ? get_user_by('email', 'Thando@kayiseit.com') : null;
            $created_by_user_id = $thando_user ? (int) $thando_user->ID : 1;
        }
...
if (!function_exists('kit_run_setup_seed')) {
    ...
    function kit_run_setup_seed(array $args = []): array
    {
        ...
        $sql_file = is_string($args['sql_file']) && $args['sql_file'] !== ''
            ? $args['sql_file']
            : (file_exists($latest_sql_file) ? $latest_sql_file : null);
        ...
        if (!$sql_file) {
            return [
                'success' => false,
                'message' => 'Seed SQL not found. Ensure latestSQL.sql exists in the plugin root.',
            ];
        }
