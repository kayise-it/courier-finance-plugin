...
function kit_employee_portal_fix_request($query_vars) {
    ...
    $slugs = array('employee-login', 'employee-dashboard');
...
function kit_employee_admin_redirect() {
    ...
    // Allow POST to admin-post.php (form submissions) and admin-ajax.php (AJAX)
    $is_post = isset($_SERVER['REQUEST_METHOD']) && $_SERVER['REQUEST_METHOD'] === 'POST';
    $uri = isset($_SERVER['REQUEST_URI']) ? $_SERVER['REQUEST_URI'] : '';
    if ($is_post && (strpos($uri, 'admin-post.php') !== false || strpos($uri, 'admin-ajax.php') !== false)) {
        return;
    }
