...
/**
 * Return JavaScript code to remove unwanted SVG circles from the page (including footer)
 */
function kit_remove_rogue_circle_js() {
    return "
(function() {
    function removeRogueCircles() {
        // Remove container if it matches progress-wrap (theme generated)
        var wraps = document.querySelectorAll('.progress-wrap');
        wraps.forEach(function(w) { w.remove(); });

        // Find and remove SVG paths that match the rogue circle pattern (M50,1 a49,49...)
        var paths = document.querySelectorAll('svg path[d^=\"M50,1 a49,49\"]');
        ...
        // Also check for any large black circles by class or role
        var rogues = document.querySelectorAll('svg[aria-hidden=\"true\"], svg.rogue-circle, svg.spinner, svg.progress-circle');
        rogues.forEach(function(svg) {
            if (svg.closest('.progress-wrap')) {
                svg.remove();
                return;
            }
            var hasPath = svg.querySelector('path[d^=\"M50,1 a49,49\"]');
            if (hasPath) {
                svg.remove();
            }
        });
    ...
    // Run immediately and also after a slight delay for lazy-loaded content
    removeRogueCircles();
    if (document.readyState === 'loading') {
        document.addEventListener('DOMContentLoaded', removeRogueCircles);
    }
    setTimeout(removeRogueCircles, 500);
    setTimeout(removeRogueCircles, 2000);
})();
    ";
}
...
function kit_employee_enqueue_portal_standalone_css() {
    ...
    // Remove unwanted SVG circles that appear in footer
...
function kit_employee_portal_enqueue_section_assets($section) {
    ...
    // small overrides to hide unexpected rogue SVGs (frontend-specific)
    wp_enqueue_style('kit-custom-overrides', $plugin_url . 'assets/css/custom-overrides.css', array('kit-tailwindcss-portal'), '1.0');
...
function kit_employee_enqueue_dashboard_assets() {
    ...
    // enqueue small custom overrides
    wp_enqueue_style('kit-custom-overrides', $plugin_url . 'assets/css/custom-overrides.css', array('kit-tailwindcss'), '1.0');
...
function kit_employee_enqueue_login_styles() {
    ...
    // ensure overrides also load for the login form
    wp_enqueue_style('kit-custom-overrides', $plugin_url . 'assets/css/custom-overrides.css', array('kit-employee-login'), '1.0');
    // Remove unwanted SVG circles
    wp_add_inline_script('jquery', kit_remove_rogue_circle_js(), 'after');
}
