                        ...
                        .done(function(response) {
                            if (response.success) {
                                ...
                                if (isModal) {
                                    ...
                                    // Regular waybill creation - go to the waybill view.
                                    // When running inside the employee portal (including iframe),
                                    // redirect the PORTAL (parent) to the portal view URL instead
                                    // of an admin.php URL that then re-renders the portal inside itself.
                                    ...
                                    if (waybillId && waybillId !== 'undefined' && waybillId !== 'null' && waybillId !== '[object Object]') {
                                        ...
                                        // Decide redirect target window (escape iframe if present)
                                        const targetWindow = (window.parent && window.parent !== window) ? window.parent : window;

                                        if (window.myPluginAjax && myPluginAjax.is_portal && myPluginAjax.portal_base) {
                                            // Employee portal context: build frontend portal URL
                                            ...
                                            console.log('🔄 Portal redirect to:', portalUrl);
                                            targetWindow.location.href = portalUrl;
                                        } else {
                                            // Fallback to original admin view URL
