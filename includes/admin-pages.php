...
/**
 * Render the plugin tools page.  Currently contains a button to seed the
 * services table on demand; more maintenance actions can be added here.
 */
function plugin_tools_page()
{
    ...
    $message = '';
    if (isset($_GET['message']) && $_GET['message'] === 'services_seeded') {
        $message = '<div class="notice notice-success"><p>Services table seeded.</p></div>';
    }
    ...
    echo '<input type="hidden" name="action" value="kit_seed_services">';
    submit_button('Seed services table');
    echo '</form>';
    echo '</div>';
}
